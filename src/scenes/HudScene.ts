...
export class HudScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    this.promptKey.setBackgroundColor(P.amber4).setPadding(6, 2, 6, 2);
    ...
    this.promptKey.on('pointerdown', () => this.game.events.emit('hud-interact'));
    this.promptText.on('pointerdown', () => this.game.events.emit('hud-interact'));
    this.root.add([this.promptKey, this.promptText]);
    this.setPrompt(null);
  ...
  private setPrompt(text: string | null): void {
    ...
    const total = this.promptKey.width + 10 + this.promptText.width;
    this.promptKey.setX(GAME_W / 2 - total / 2);
    this.promptText.setX(GAME_W / 2 - total / 2 + this.promptKey.width + 10);
  }
